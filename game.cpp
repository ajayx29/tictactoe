...
//to get the player input and update the board
...
bool gameover(){
    //check the win for Simple Rows and Simple Column
    for(int i=0; i<3; i++)
    if(board[i][0] == board[i][1] && board[i][0] == board[i][2] || board[0][i] == board[1][i] && board[0][i] == board[2][i])
    return false;

    //check the win for both diagonal
    ...
    //Check the if game already draw
    draw = true;
    return false;
}
